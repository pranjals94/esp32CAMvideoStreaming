...
//	.frame_size = FRAMESIZE_QVGA,
	.frame_size = FRAMESIZE_UXGA, //UXGA = 16600 by 1200 resolution max supported by ov2640CameraSensor
...
esp_err_t toggleFlash(httpd_req_t *req){
//	state = !state; // toggle
	esp_err_t error;
	ESP_LOGI(TAG, "LED Turned Toggled");
...
httpd_uri_t uri_handler_toggle_flash = {
    .uri = "/toggleFlash",
    .method = HTTP_GET,
    .handler = toggleFlash};
...
httpd_handle_t start_webserver(void)
{
  ...
  if (httpd_start(&server, &config) == ESP_OK)
  {
    ...
    httpd_register_uri_handler(server, &uri_handler_toggle_flash);
    return server;
  }
...
static void configure_Led_Flash(void){

	gpio_reset_pin(Led_flash);
	gpio_set_direction(Led_flash, GPIO_MODE_OUTPUT);
	gpio_set_level(Led_flash, false);
}
...
//  configure_Led_Flash(); //flash led needs more power
